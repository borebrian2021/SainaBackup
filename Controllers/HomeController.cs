...
namespace SHOP.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult list_of_items()
        {
            ...
            var countItems = confirm_item.Count();
            var toLoop = confirm_item;
            foreach (var item in confirm_item)
            {
                var check_details = _context.Shop_items.FirstOrDefault(x => x.id == item.id);
                Random k = new Random();
                int random = k.Next(000000, 999999);

                if (check_details.Quantity < item.Quantity)
                {
